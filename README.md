...
> 本仓库不附带演示 GIF/截图，跑起对应脚本即可在你本机看到实际效果。
...
│   └── push_dataset_act.py     #   [可选] 把采集好的数据集上传到 Hugging Face
│
...
- **纯命令行服务器即可**：训练脚本（`train_act.py`、`train_vla.py`）、上传脚本（`push_dataset_*.py`）。

> 如果只有一台无显示器的 GPU 服务器，可以在本地有屏幕的机器上采集数据，把数据拷到服务器训练；也可以用 `push_dataset_act.py` / `push_dataset_language.py` 把数据上传到自己的 Hugging Face 账号，训练机上再 `git clone` 拉回来。
...
> 注意：Hugging Face 上流传的 `Jeongeun/omy_pnp*` 系列是给旧版 6 自由度 OMY 机械臂采的，动作维度是 7（Franka 是 8），**不能直接拿来在本仓库用，会因为维度不匹配报错或产生错误行为**。采集自己的新数据后，可以用 `push_dataset_act.py` / `push_dataset_language.py` 上传到自己的 Hugging Face 账号（见[第五章](#五完整工作流程与运行步骤)）。
...
> python vla/train_vla.py --config_path config/pi0_franka.yaml # 训练 π0（需 GPU）
...
```bash
python act/collect_data_act.py      # 需要显示器；弹出窗口后用键盘遥操作，按上方按键表操作
```
...
4. 训练循环 3000 步（可按需调大，5000 步以上结果更值得评估）。
5. `policy.save_pretrained(...)` 把检查点存到 `./ckpt/act_franka`。
6. 评估：用 `EpisodeSampler` 逐帧推理，计算预测动作与真值动作（8 维：7 关节角 + 1 夹爪）的平均绝对误差并绘图对比。

### 步骤 4：部署 ACT 策略（`deploy_act.py`）

```bash
python act/deploy_act.py    # 需要显示器；从 ./ckpt/act_franka 加载模型并自动执行
```

把训练好的 ACT 检查点放回仿真自动执行（rollout）：

1. 构建特征与 `ACTConfig`，从 `./ckpt/act_franka` 加载策略到 GPU。
2. 加载 MuJoCo 抓放环境并 `reset`。
3. 推演循环（20 Hz）：取观测（末端状态 + 相机图，缩放到 256×256）→ 组装成 observation 字典 → `policy.select_action(obs)` 推理动作 → 环境 `step` 执行 → `render` 渲染 → `check_success` 判定成功（这里才真正用到自动成功检测）。
...
```bash
python vla/train_vla.py --config_path config/pi0_franka.yaml   # 训练 π0，检查点存到 ./ckpt/pi0_franka
```
...
```bash
python vla/deploy_pi0.py    # 需要显示器；从 ./ckpt/pi0_franka/checkpoints/last/pretrained_model 加载
```
...
```bash
python vla/train_vla.py --config_path config/smolvla_franka.yaml   # 训练 SmolVLA，检查点存到 ./ckpt/smolvla_franka
```
...
```bash
python vla/deploy_smolvla.py    # 需要显示器；从 ./ckpt/smolvla_franka/checkpoints/last/pretrained_model 加载
```
...
### 数据集/策略上传到 Hugging Face（`push_dataset_act.py` / `push_dataset_language.py`）
...
python act/push_dataset_act.py           # 上传 ./demo_data
...
`config/pi0_franka.yaml` / `config/smolvla_franka.yaml` 供 `train_vla.py` 读取（通过 `--config_path` 指定），字段含义：

```yaml
dataset:
  repo_id: franka_pnp_language      # 数据集仓库 ID
...
output_dir: ./ckpt/pi0_franka       # 检查点保存目录
batch_size: 16
job_name: pi0_franka
...
wandb:
  ...
  project: pi0_franka
  entity: <你的wandb账户名>          # 改成你自己的 wandb 账户名
  disable_artifact: true
```

> 改用自己的数据时，主要改 `repo_id` / `root` / `output_dir`，并把 `wandb.entity` 换成你的账户（或把 `wandb.enable` 设为 `false` 改为本地日志）。`smolvla_franka.yaml` 额外多一行 `policy.device: cuda`，其余字段含义相同。
...
A：遥操作与带渲染窗口的脚本需要图形界面，请在本地有显示器的机器上跑。纯命令行服务器只适合 `train_vla.py`/`train_act.py` 训练和上传脚本。
...
A：数据集不用自己采——本项目已把采集好的 Franka 数据集开源在 [a3124371940/franka_pnp](https://huggingface.co/datasets/a3124371940/franka_pnp)（ACT）和 [a3124371940/franka_pnp_language](https://huggingface.co/datasets/a3124371940/franka_pnp_language)（语言条件），`git clone` 下来把脚本里的 `root` 指过去就能直接回放/训练。**预训练权重（checkpoint）目前没有现成的**，训练这一步仍需要自己用 GPU 跑。另外提醒：Hugging Face 上的 `Jeongeun/omy_pnp*` 系列是给旧版 OMY 机械臂做的（动作是 7 维，本项目 Franka 是 8 维），维度不兼容，不能直接拿来用。自己采集的新数据可以用 `push_dataset_act.py` / `push_dataset_language.py` 传到自己的 Hugging Face 账号备份或分享。

**Q5：采集多少条演示数据比较合适？**
A：ACT（单一固定任务）建议 50~100 条；语言条件数据集建议红/蓝两种指令各 30~50 条，总量 60~150 条左右。先用 10~20 条跑通"采集→回放→训练→部署"全流程确认没问题，再回头采到正式规模，比一次性采很多但流程有 bug 更省时间。

**Q6：MuJoCo 版本有要求吗？**
A：必须 **3.1.6**，其他版本可能与本项目的解析器/资源不兼容。

**Q7：ACT、π0、SmolVLA 该选哪个入门？**
...
> 本文档随代码注释一同维护：`mujoco_env/` 下所有 Python 模块、`act/`、`vla/` 下的教程脚本，注释/说明均已中文化，可结合源码对照阅读。
