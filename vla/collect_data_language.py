...
REPO_NAME = 'omy_pnp_language'      # 数据集名字（repo id），后续训练时按这个名字找数据
...
if create_new:
    ...
    dataset = LeRobotDataset.create(
                ...
                image_writer_threads=10,            # 后台写图片用的线程数（多线程加速存盘）
                image_writer_processes=5,           # 后台写图片用的进程数
        )
else:
...
# --- 准备主循环用到的几个变量 ---
action = np.zeros(7)        # 当前动作，先初始化为「全 0」（即不动）。7 维 = 6 个关节角度 + 1 个夹爪
...
while PnPEnv.env.is_viewer_alive() and episode_id < NUM_DEMO:
    ...
    if PnPEnv.env.loop_every(HZ=20):    # 控制采集节奏：每秒只执行 20 次下面的逻辑（对应 fps=20）
        ...
        joint_q = PnPEnv.step(action)       # 真正执行这一帧动作，并返回机器人当前的关节状态
        action = PnPEnv.q[:7] # 6 个关节角度和 1 个夹爪   # 取出实际达到的状态作为「这一帧的动作标签」
        action = action.astype(np.float32)              # 统一成 float32 类型（数据集要求）
        if record_flag:
            ...
            dataset.add_frame( {
                    ...
                    "observation.state": joint_q[:6],           # 机器人状态（6 个关节角度）
                    "action": action,                           # 这一帧的动作（6 关节 + 1 夹爪）
