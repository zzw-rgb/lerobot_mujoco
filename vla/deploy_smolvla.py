...
（数据集 = 一堆“看到的画面 + 当时的指令 + 机器人当时做的动作”的录像，
 模型就是靠模仿这些录像学会任务的。这里是别人已经采集好、上传到云端的现成数据。）
'''
# 下面这行用 git 把数据集从 Hugging Face（一个托管模型/数据集的网站）克隆到本地。
# 同样请在终端手动执行；如果你已经有数据，可以跳过。
...
# 这样无论数据集放在 './demo_data_language' 还是 './omy_pnp_language' 都能加载成功。
try:
...
# 策略通过一个配置类来初始化，本例中即 `DiffusionConfig`。在这个示例里，
...
# - 第一个参数是权重文件夹路径：'./ckpt/smolvla_omy/checkpoints/last/pretrained_model'
...
# 如果你没有足够的资源进行训练，也可以直接从 hub 加载训练好的策略。
# （即不用自己训练，直接从 Hugging Face 云端下别人练好的权重，把上面那行换成下面这行即可。）
...
while PnPEnv.env.is_viewer_alive():  # 只要仿真窗口还开着（没被关掉），就一直循环
    ...
    if PnPEnv.env.loop_every(HZ=20):
        ...
        if success:
            ...
            save_image = False
        # ---------- 第 1 步：感知（收集模型需要的输入）----------
        # 获取环境的当前状态：机械臂各关节当前的角度。取前 6 个 [:6]，对应 6 个关节。
        ...
        # 模型输出的是一批/多维结果，这里取出第 0 条样本的前 7 个数值 [0,:7]
        # （对应 6 个关节角 + 1 个夹爪开合），并搬回 CPU、脱离计算图、转成普通 numpy 数组，
        # 这样才能交给仿真环境去执行。
