...
训练 SmolVLA 请在终端运行（30k steps，输出到 ckpt/smolvla_franka_v2）：
...
推荐统一部署入口（也支持 --headless）：
    CUDA_VISIBLE_DEVICES=0 python vla/deploy_vla.py --config_path=config/vla/smolvla_franka.yaml --checkpoint=./ckpt/smolvla_franka_v2/checkpoints/030000/pretrained_model --device=cuda --seed=0 --max_steps=2000
...
# [终端命令] CUDA_VISIBLE_DEVICES=0 python vla/train_vla.py --config_path=config/vla/smolvla_franka.yaml
...
# - chunk_size=50：模型一次预测未来 50 步（20 Hz 下约 2.5 秒）的动作。
# - n_action_steps=10：只执行前 10 步（0.5 秒）就重新观察，减少开环累计误差。
cfg = SmolVLAConfig(input_features=input_features, output_features=output_features, chunk_size=50, n_action_steps=10)
...
# - 第一个参数是权重文件夹路径：'./ckpt/smolvla_franka_v2/checkpoints/last/pretrained_model'
#   就是步骤 2 训练完保存下来的“成果”，里面是学好的网络参数。
# - dataset_stats 把数据集统计量交给模型，让它知道怎么归一化输入、还原输出。
# 这一步之后，policy 就是一个“学成归来、可以直接干活”的模型对象了。
policy = SmolVLAPolicy.from_pretrained('./ckpt/smolvla_franka_v2/checkpoints/last/pretrained_model', dataset_stats=dataset_metadata.stats)
...
while PnPEnv.env.is_viewer_alive():  # 只要仿真窗口还开着（没被关掉），就一直循环
    ...
    if PnPEnv.env.loop_every(HZ=20):
        ...
        if success:
            ...
            save_image = False
        # ---------- 第 1 步：感知（收集模型需要的输入）----------
        # 新数据使用 7 关节 + 夹爪状态；按检查点维度切片以兼容旧模型。
