...
# ======================================================================
# ## 步骤 1. 修改配置文件 config/pi0_franka.yaml
# ======================================================================

# ======================================================================
# config/pi0_franka.yaml 文件
# ```
# dataset:
#   repo_id: franka_pnp
#   root: ./franka_pnp
...
# [终端命令] python vla/train_vla.py --config_path config/pi0_franka.yaml
...
policy = PI0Policy.from_pretrained('./ckpt/pi0_franka/checkpoints/last/pretrained_model', dataset_stats=dataset_metadata.stats)
...
while PnPEnv.env.is_viewer_alive():   # 只要可视化窗口还开着（没被关掉），就一直循环。
    ...
    if PnPEnv.env.loop_every(HZ=20):  # 控制“决策频率”：约每秒 20 次（20Hz）才执行一次下面的决策逻辑。
        ...
        if success:
            ...
            step = 0
            save_image = False
        # 获取环境的当前状态：取机器人前 7 个关节的角度，作为“本体感知”观测交给模型。
