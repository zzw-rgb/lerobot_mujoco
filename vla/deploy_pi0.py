...
训练 π0 请在终端运行（40k steps，输出到 ckpt/pi0_franka_v2）：
...
推荐统一部署入口（也支持 --headless）：
    CUDA_VISIBLE_DEVICES=0 python vla/deploy_vla.py --config_path=config/vla/pi0_franka.yaml --checkpoint=./ckpt/pi0_franka_v2/checkpoints/040000/pretrained_model --device=cuda --seed=0 --max_steps=2000
...
'''
数据集（一堆“演示录像”：人/专家先示范怎么做任务，模型从中学会模仿）由
vla/collect_data_language.py 采集得到，带语言指令，用于训练“听话”的 VLA 策略。
已开源一份现成数据集，不想自己采集可以直接下载：
...
# ======================================================================
# ## 步骤 1. 修改配置文件 config/vla/pi0_franka.yaml
# ======================================================================

# ======================================================================
# config/vla/pi0_franka.yaml 文件
# ```
# dataset:
#   repo_id: franka_pnp
#   root: ./franka_pnp
...
# output_dir: ./ckpt/pi0_franka_v2
# batch_size: 8
# job_name: pi0_franka_v2
# resume: false
# seed : 42
# num_workers: 8
# steps: 40_000
...
# [终端命令] CUDA_VISIBLE_DEVICES=0 python vla/train_vla.py --config_path=config/vla/pi0_franka.yaml
...
# 找不到就退而求其次用 ./franka_pnp_language，哪个存在就用哪个，避免因路径不同而报错。
try:
...
#   - chunk_size=50     ：模型一次预测未来 50 步（20 Hz 下约 2.5 秒）的动作块。
#   - n_action_steps=50 ：当前锁定的 LeRobot π0 要求它与 chunk_size 相同。
cfg = PI0Config(input_features=input_features, output_features=output_features, chunk_size=50, n_action_steps=50)
...
policy = PI0Policy.from_pretrained('./ckpt/pi0_franka_v2/checkpoints/last/pretrained_model', dataset_stats=dataset_metadata.stats)
# 若已用 vla/push_dataset_language.py 之外的方式把自己训练好的策略也上传到了
# Hugging Face（policy.push_to_hub），可以像下面这样直接从远端仓库加载，换成自己的 repo_id：
# policy = PI0Policy.from_pretrained("<你的用户名>/franka_pnp_pi0", config=cfg, dataset_stats=dataset_metadata.stats)
...
while PnPEnv.env.is_viewer_alive():   # 只要可视化窗口还开着（没被关掉），就一直循环。
    ...
    if PnPEnv.env.loop_every(HZ=20):  # 控制“决策频率”：约每秒 20 次（20Hz）才执行一次下面的决策逻辑。
        ...
        if success:
            ...
            step = 0
            save_image = False
        # 新数据使用 7 关节 + 夹爪状态；旧检查点仍只需要前 7 维。
        state_feature = policy.config.input_features['observation.state']
        state = PnPEnv.get_joint_state()[: int(state_feature.shape[0])]
        ...
        # 模型输出是 GPU 上的张量；这里取第 0 个样本的前 8 维（7 关节角 + 1 夹爪，机器人需要的动作维度），
...
# 只有当你真的想公开/备份自己的模型时，才取消注释并换成自己的仓库名来用。
# policy.push_to_hub(
#     repo_id='<你的用户名>/franka_pnp_pi0',
