...
从 ./ckpt/act_y 加载检查点，在 MuJoCo 抓放环境中自动执行并判断成功。

运行方式（需要 GPU；无 GPU 可下载作者提供的预训练检查点）：
...
# 策略通过一个配置类来初始化，本例中为 `DiffusionConfig`。在这个示例里，
...
# from_pretrained('./ckpt/act_y', ...)：这一步就是"加载训练好的检查点"——
# 把之前 train_act.py 训练出来、存在 ./ckpt/act_y 文件夹里的模型权重读回来，得到一个能用的策略。
