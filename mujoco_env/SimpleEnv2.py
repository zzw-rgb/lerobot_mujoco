...
class SimpleEnv2:
    ...
    def reset(self, seed = None):
        ...
        # 数据集中的 action 必须保存本帧真正下发的目标命令，而不是控制执行前的
        # 当前关节位置。compute_q/get_command_action 专门维护这份动作标签。
    ...
    def get_command_action(self):
        """返回最近一次 ``step`` 下发的 7 关节目标和归一化夹爪命令。"""
    ...
    def render(self, teleop=False, idx = 0):
        ...
        if getattr(self, 'instruction', None) is not None:
            ...
            # loc='top' 时 text1/text2 会在同一行居中叠着画，两栏同时给字会互相压字；
            # 只用 text2 传一整行文本，text1 留空可避免重叠。
            self.env.viewer_text_overlay(text1='',text2='Language Instructions: %s'%(language_instructions),loc='top')
        self.env.render()
    ...
    def check_success(self):
        ...
        mug_dofadr = self.env.model.body(self.obj_target).dofadr[0]
